...
test: bin/nosetests
	bin/nosetests -s grape/raisin

coverage: bin/coverage bin/nosetests
	bin/nosetests --with-coverage --cover-html --cover-html-dir=html --cover-package=grape.raisin
	bin/coverage html

pylint:	bin/pylint
	bin/pylint -i y grape/raisin

flake8:	bin/flake8
	bin/flake8 --max-complexity 12 grape/raisin

pep8:	bin/pep8
	bin/pep8 grape/raisin

pyflakes:	bin/pyflakes
	bin/pyflakes grape/raisin
...
templer: bin/python
	...
	export OLDHOME="${HOME}"; export HOME="${PWD}"; ./bin/templer dotpackage grape.raisin; export HOME="${OLDHOME}"

diff: bin/python
	# Show the difference between the current package and the regenerated one
	colordiff -c -r grape.raisin .|less -r

sloccount:	bin/python
	sloccount grape/raisin
